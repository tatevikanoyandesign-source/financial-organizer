...
const description = 'Track spending progress against a category budget limit.'

const budgetStates = [
  { title: 'On track', item: budgets[0] },
  { title: 'Near limit', item: budgets[1] },
  { title: 'Over budget', item: budgets[3] },
]

const sections: DocSectionConfig[] = budgetStates.map(({ title, item }) => ({
  title,
  description: `${item.category} — $${item.spent} of $${item.limit} spent.`,
  width: 'card' as const,
  code: `import { BudgetCard } from '@/components/financial/budget-card'
...
const meta = {
  ...
  parameters: {
    ...
    docs: {
      ...
      page: createComponentDocPage(BudgetCard, description, sections),
    },
  },
